...
for i in range(len(fips)):
    ...
    y_na = []
    for y in year:
        y_he.append(np.sum(data['DrugReports'][(data.SubstanceName=='Heroin') & (data.FIPS_Combined==f) & (data.YYYY==y)]))
        y_na.append(np.sum(data['DrugReports'][(data.SubstanceName!='Heroin') & (data.FIPS_Combined==f) & (data.YYYY==y)]))
    y_he = np.array(y_he)
    y_na = np.array(y_na)
    he_list.append(y_he)
    na_list.append(y_na)
...
for s in state:
    ...
    for d in sorted(list(set(data['SubstanceName'][data.State==s]))):
        drug_trend = []
        for y in year:
            drug_trend.append(np.sum(data['DrugReports'][(data.State==s) & (data.YYYY==y) & (data.SubstanceName==d)]))
        '''
        if drug_trend[0] == 0:
            ...
            if np.sum(tmp) == 0:
                ...
                for c in county:
                    if np.sum(data['DrugReports'][(data.State==s) & (data.YYYY==y) & (data.SubstanceName==d) & (data.COUNTY==c)]) > 0:
...
for d in need_deal:
    ...
    y1 = year[-2]
    for f in fips:
        report1 = np.sum(data['DrugReports'][(data.FIPS_Combined==f) & (data.SubstanceName==d) & (data.YYYY==y1)])
        report2 = np.sum(data['DrugReports'][(data.FIPS_Combined==f) & (data.SubstanceName==d) & (data.YYYY==y2)])
        if len(list(data['DrugReports'][(data.FIPS_Combined==f) & (data.SubstanceName==d) & (data.YYYY==y1)])) > 1:
            print(error)
