    ...
    for i in range(len(all_fips)): 
        if np.sum(np.abs(result[i])) > 0:
            ...
            for y in list(range(2010,2018)):
                ori.append(np.sum(data['DrugReports'][(data.SubstanceName==d) & (data.FIPS_Combined==f) & (data.YYYY==y)]))
            to_plot = to_plot + ori
            #print(to_plot)
...
for ind in range(len(files)):
    ...
    for state in states:
        ...
        file_write.write(str(state)+','+files[ind][:-4]+' first reported in '+str(first_year)+' in '+str(county_list)+'\n')
